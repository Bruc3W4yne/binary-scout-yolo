...
```text
A scout-guided selected-tile pipeline is implemented and measurable. On the current full-val tile-label evaluation, the tiny spatial MLP scout covers 0.516 of object centers at K=8, compared with 0.452 +/- 0.004 for 5-trial random K=8, 0.469 for a train-split spatial prior, and 0.953 for greedy-oracle K=8.
```
...
| Binary core | Pass | `python scripts\verify_packed_kernel.py --include-nonbinary` passed `single-channel-1x1`, `u8-sized-3x3`, `rgb-bitplanes-3x3`, `u64-full-3x3`, `threshold_i32_to_u8`, `shape_guard_errors`, and nonbinary 0/255 plane handling. Clean commit `b19ad32` also built `kernel.dll`, extracted a one-image binary-XNOR feature cache, and verified metadata for filters, kernel size, threshold, seed, input channels, and weight hash. | Current kernel is CPU/OpenMP C through ctypes, not CUDA. That is fine for the binary-scout claim, but not a 4090 acceleration claim. |
...
| Scout pipeline | Pass | Feature caches exist for `bitplane-stats`, spatial bitplane features, and a 25-image binary-XNOR smoke cache. `python scripts\verify_tile_features.py --feature-file data\tile_features\bitplane_stats_spatial_val.npz --expect-feature-dim 38 --expect-feature-mode bitplane-stats-spatial` passed. `evaluate_scout_recall.py` with `runs\scout_spatial_mlp\scout_bitplane_stats_spatial.pt` reached K=8 object recall `0.516`. Clean commit `c1418b8` also verified random multi-trial, train-prior, heuristic, oracle-count, and oracle-greedy baselines. | Full train/val binary-XNOR scout training has not been run yet. Current best scout is bitplane/spatial MLP, not binary-XNOR. |
| Detector/router | Pass | `scripts\run_yolo_tiles.py` supports `full`, `all`, `random`, `prior`, `heuristic`, `oracle-count`, `oracle-greedy`, `scout`, and `scout-live`. Clean commit `c1418b8` ran one-image Windows/CUDA smoke tests for `full`, `all`, `random`, `prior`, `heuristic`, `oracle-greedy`, and cached `scout`. Clean commit `baf87f3` verified CUDA-synchronized timing phases and one warmup image. | The YOLO smoke benchmark uses COCO-pretrained `yolov8n.pt`, so class-aware VisDrone mAP is not final. |
...
| Code-quality pass | Pass for current handoff | Stale `PROD.md`, legacy planning docs, legacy demo scripts, and the out-of-scope full-BNN detector experiment were deleted. Feature cache schema is compact, binary wrapper guards are in place, and README commands were updated. | Script imports still use local `sys.path` insertion instead of editable package installation. That is acceptable for a school CLI repo but should be cleaned before packaging. |
| Handoff docs | Pass | `README.md`, `docs/current_results.md`, `docs/evaluation_protocol.md`, `docs/final_project_claims.md`, `docs/binary_xnor_core.md`, and this audit describe the runnable path, current claims, commands, and caveats. | `docs/next_goal.md` remains the final acceptance contract until the goal is closed. `docs/pro_final_review.md` is historical review context, not command truth. |
...
python scripts\evaluate_scout_recall.py --features data\tile_features\bitplane_stats_spatial_val.npz --mode random --random-trials 5 --top-k-values 3 5 8
python scripts\evaluate_scout_recall.py --features data\tile_features\bitplane_stats_spatial_val.npz --mode prior --prior-features data\tile_features\bitplane_stats_spatial_train.npz --top-k-values 3 5 8
python scripts\evaluate_scout_recall.py --features data\tile_features\bitplane_stats_spatial_val.npz --mode oracle-greedy --top-k-values 3 5 8
...
python scripts\run_yolo_tiles.py --selector scout --top-k 8 --split val --max-images 1 --warmup-images 1 --device cuda --weights yolov8n.pt --features data\tile_features\bitplane_stats_spatial_val.npz --checkpoint runs\scout_spatial_mlp\scout_bitplane_stats_spatial.pt
```
