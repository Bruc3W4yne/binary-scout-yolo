...
| Detector/router | Pass | `scripts\run_yolo_tiles.py` supports `full`, `all`, `random`, `prior`, `heuristic`, `oracle-count`, `oracle-greedy`, `scout`, and `scout-live`. Clean commit `c1418b8` ran one-image Windows/CUDA smoke tests for `full`, `all`, `random`, `prior`, `heuristic`, `oracle-greedy`, and cached `scout`. | The YOLO smoke benchmark uses COCO-pretrained `yolov8n.pt`, so class-aware VisDrone mAP is not final. |
